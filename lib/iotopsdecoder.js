...
function decodeSerialData(data, origin, time, aps, options) {
  ...
  data.forEach((transmitter) => {
    if(typeof transmitter.usbAddress?.hwId === 'string') {
      let receiverId = transmitter.usbAddress.hwId;
 ...
 * @param {Object} options The packet decoding options.
 */
function decodeApInventory(data, origin, time, aps, options) {
  let infrastructureMessages = [];

  data.apInventories.forEach((inventory) => {
    let deviceId = inventory.mac.toString('hex');
    infrastructureMessages.push({
        deviceId: deviceId,
        deviceIdType: Raddec.identifiers.TYPE_EUI48,
        isHealthy: (inventory.status === 1), // TODO: confirm status codes
        timestamp: time
        // TODO: location?
    });
  });
